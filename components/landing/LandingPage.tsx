...
const nameGuides = [
  ["Dog name ideas", "Popular, cute, gender-specific, and breed-inspired ideas.", "/dog-names"],
  ["Cat name ideas", "Find cute, color-based, and characterful names for cats.", "/cat-names"],
  ["Big dog names", "Bold ideas for big and giant-breed companions.", "/big-dog-names"],
  ["Long dog names", "Full-length names with easy everyday nicknames.", "/long-dog-names"],
  ["Unique pet names", "Less common ideas for every kind of companion.", "/unique-pet-names"],
  ["Funny pet names", "Playful names that bring a smile to your home.", "/funny-pet-names"],
];
...
export function LandingPage() {
  ...
  return <>
    ...
    <main id="top">
      ...
      <section className="section seo-related" aria-labelledby="name-guides"><Container><SectionHeading eyebrow="PET NAME GUIDES" title="Explore pet name ideas by personality, type, and style." lead="Start with a focused name guide when you have a particular kind of pet or naming direction in mind." /><nav className="seo-link-grid" id="name-guides" aria-label="Pet name guides">{nameGuides.map(([title, description, href]) => <a href={href} key={href}><strong>{title}</strong><span>{description}</span></a>)}</nav></Container></section>
